...
def scrape():
    ...
    file = open("testOnlyFirstPages.txt", "a")
    errorLog = open("error_log.txt", "a")
    # keyword = "Microsoft"
    # ticker = "MSFT"
    # listOfKeywords = ["Apple", "Amazon", "Tesla", "Alphabet", "Facebook", "NVIDIA", "Berkshire", "JPMorgan", "Johnson", "UnitedHealth"]
    listOfKeywords = ["Netflix", "Paypal", "Salesforce", "Exxon", "Comcast", "Pfizer", "Verizon", "CapitalOne", "Cisco"]
    # listOfTickers = ["APPL", "AMZN", "TSLA", "GOOGL", "FB", "NVDA", "BRK.B", "JPM", "JNJ", "UNH"]
    listOfTickers = ["NFLX", "PYPL", "CRM", "XOM", "CMCSA", "PFE", "VZ", "COF", "CSCO"]
    numPages = 1

    for idx_cmp, t in enumerate(listOfTickers):

        for index, d in enumerate(listOfDates):
            count = 0
            print(index, d, t)
            pages = paginate("https://www.google.com/search?", listOfKeywords[idx_cmp], d, numPages, count)
            ...
            for soup in pages:
                try:
                    for data in soup.findAll('a', class_='WlydOe'): # <a> tag
                        ...
                        file.write(f'{d}|{t}|{title}|{snippet}|{link}')
                        file.write('\n')
                        # print(f'Title: {title}')
                        # print(f'Link: {link}')
